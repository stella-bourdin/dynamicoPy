...
def identify_ET(tracks, NH_lim, SH_lim, lon_name="longitude", minus_3h=True, fill = True):
    ...
    tracks["lat_STJ_NH"] = NH_lim.sel(time=target_time, longitude=target_lon, method = "nearest")
    tracks["lat_STJ_SH"] = SH_lim.sel(time=target_time, longitude=target_lon, method = "nearest")
    tracks["ET"] = (tracks.lat > tracks.lat_STJ_NH) | (tracks.lat < tracks.lat_STJ_SH)

    if fill :
        # Fill trajectories once one point is ET
        first_times = tracks[tracks.ET == True].sort_values("time").groupby("track_id").time.first()
        t_ET = tracks[tracks.track_id.isin(first_times.index)] # Track that are labelled ET at one point
        t_noET = tracks[~tracks.track_id.isin(first_times.index)] # Track that are not labelled ET at one point
