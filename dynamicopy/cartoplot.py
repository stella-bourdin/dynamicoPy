...
def plot_tracks(tracks_file="tests/tracks.csv", projection=ccrs.Robinson()):
    """
    Function to plot tracks from the tracks.txt file generated by TempestExtremes

    Parameters
    ----------
    tracks_file : Path to the tracks.txt file
    ...
    count = df.groupby("track_id").count()
    to_drop = count[count[" year"] > 8000].index
    df = df[~df.track_id.isin(to_drop)]
    clusters = df.track_id.unique()
    ...
    for c in clusters:
        cluster = df[df.track_id == c]
        lon = cluster[" lon"].values
