...
def plot_tracks(tracks,
    ...
    if draw_lines:
        for id in tracks[id_col].unique():
            ...
            ax.plot(lon, lat, transform = ccrs.PlateCarree(), c = 'k', alpha = 0.7, linewidth = 0.5)

    sns.scatterplot(data = tracks, x = lon_col, y= lat_col, hue=intensity_col, ax = ax, transform=ccrs.PlateCarree(),
                    palette = cmap, size = intensity_col, sizes = (4, 20))
    return fig, ax
